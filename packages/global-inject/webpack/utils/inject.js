...
const { generateStyleCode, generateHtmlCode } = require('./pages')
...
const pageInjectCode = ({ sourceCompiler, injectCompiler, curPage }) => {
  ...
  const injectDataProperties = injectScript
    ? extractDataProperties(injectScript?.content)
    : []
  ...
  for (const objectType of OBJECT_PROPERTIES) {
    if (injectScript && injectScript.content) {
