...
/* pages.json file */
"injectLoader": {
  /* 同一全局组件 injectTags or injectCode 二选一 */
  "injectTags": ["custom-global-component"], // 以标签的形式插入代码
...
"pages": [
  {
    ...
    "style": {
      ...
      "injectTags": ["custom-global-component"], // 注入标签
      "rootEle":"div"
    }
  }
]
...
✔ `injectCode`、`injectTags`、`rootEle` 支持在单独页面的style里配置，优先级高于全局配置


### 两种注入方式的区别
- injectTags
  - `injectTags` 会将组件的 `<custom-global-component />` 代码注入到页面的 `template` 中，从而实现全局组件的引入
...
### tips
- 优先推荐使用 `injectTags` 配置，因为这种方式不会导致代码冲突，对原有业务代码的侵入性最小
